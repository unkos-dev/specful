...
| `release-plz.yml` | `release-pr`, `release` | Maintains a rolling release pull request from merged conventional commits (version bump, `CHANGELOG.md`); on merge, publishes to crates.io and creates the git tag and GitHub release. Standalone: it triggers on push to `main`, not `pull_request`. |
...
| `RELEASE_PLZ_APP_ID`, `RELEASE_PLZ_APP_PRIVATE_KEY` | `release-plz` / `release-pr`, `release-plz` / `release` | The GitHub App token mint fails, so the run fails outright. No pull request gate is affected: `release-plz.yml` carries no required context. |
| `CARGO_REGISTRY_TOKEN` | `release-plz` / `release-pr`, `release-plz` / `release` | The crates.io publish step fails, so the `release` run fails (the rolling PR itself can still open and update, since only the `release` job publishes). No pull request gate is affected. |
