...
Renovate reads that config file rather than the generated one. Custom managers track the annotated `cargo-dist-version`
and action pins, while the `github-actions` manager ignores `release.yml`. The `.mise.toml` and `dist-workspace.toml`
cargo-dist pins move together in one pull request because dist refuses to run when those versions differ. Regenerate
`release.yml` on the Renovate branch; the required `plan` job in the Release workflow rejects stale output.
