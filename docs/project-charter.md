...
Optional harness-side skills teach artifact craft, help plan and execute implementation work, expose CLI operations, and
provide structured substantive review. They remain integration surfaces, not canonical project knowledge. Adopting
maintainers decide whether review is advisory or required and own the mechanism that enforces that choice.
...
- opt-in harness-side skills for artifact-specific authoring, implementation planning and execution, substantive review,
  validation, indexing, and retrieval, installed once per user and never written into an adopting repository.

Planned after the initial release: traceability views and verification and code-ownership relationship types.
