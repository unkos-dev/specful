...
5. Run `specful index` to regenerate the navigation views, then `specful validate`, and commit both the source documents
   and the regenerated views together. A repository formatter that rewrites JSON or Markdown needs an ignore entry for
   `.specful/generated/` in the same commit, because a reformatted catalog no longer matches its sources.
