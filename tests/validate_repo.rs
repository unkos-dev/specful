...
fn does_not_delete_a_marker_bearing_file_reached_through_a_symlink() {
    ...
    let outside = tempfile::tempdir().expect("create outside scratch");
    ...
    std::os::unix::fs::symlink(outside.path(), scratch.join("docs/specs/linked"))
...
fn does_not_write_an_expected_index_through_a_symlink() {
    ...
    let outside_dir = tempfile::tempdir().expect("create outside scratch");
    let outside = outside_dir.path().join("outside-index");
    let original = "<!-- Generated by specful index. Do not edit. -->\n\n# outside\n";
...
fn does_not_write_the_catalog_through_a_symlink() {
    ...
    let outside_dir = tempfile::tempdir().expect("create outside scratch");
    let outside = outside_dir.path().join("outside-catalog");
...
fn does_not_write_the_catalog_through_a_symlinked_parent() {
    ...
    std::os::unix::fs::symlink(outside.path(), &generated).expect("create parent symlink");
...
/// The returned guard's atomic, randomized creation and RAII cleanup on
/// drop replace a hand-rolled unique name plus manual removal.
fn copy_fixture(name: &str, label: &str) -> tempfile::TempDir {
    fn copy_tree(from: &Path, to: &Path) {
    ...
    let root = tempfile::Builder::new()
        .prefix(&format!("specful-{label}-"))
        .tempdir()
        .expect("create scratch");
    copy_tree(&fixture(name), root.path());
    root
}
...
fn rejects_an_artifact_file_replaced_by_a_symlink_outside_the_repository() {
    ...
    let outside_dir = tempfile::tempdir().expect("create outside scratch");
    let outside = outside_dir.path().join("specful-symlink-final-target");
...
fn reports_unreadable_directories() {
    ...
    /// Restores readable permissions even when the assertions panic, so the
    /// TempDir guard's own Drop can remove the tree afterward.
    ...
    let locked = root.path().join("docs/adr");
    std::fs::set_permissions(&locked, std::fs::Permissions::from_mode(0o000))
        .expect("remove directory permissions");
    let _restore = Restore(locked);

    let findings = validate_repository(root.path());
    let rendered: Vec<String> = findings.iter().map(|f| f.render()).collect();
    assert!(
        rendered
            .iter()
