...
#[test]
fn index_commands_preserve_indexes_with_unsupported_encodings() {
    for relative in ["docs/specs/system/index.md", "docs/specs/ghost/index.md"] {
        for bytes in [
            [0xff, 0xfe]
                .into_iter()
                .chain("Human notes".encode_utf16().flat_map(u16::to_le_bytes))
                .collect::<Vec<_>>(),
            b"\xef\xbb\xbf<!-- Generated by specful index. Do not edit. -->\n".to_vec(),
        ] {
            let scratch = copy_fixture("valid-repo", "protected-encoding");
            let file = scratch.path().join(relative);
            std::fs::create_dir_all(file.parent().unwrap()).unwrap();
            std::fs::write(&file, &bytes).unwrap();
            for mut args in [vec!["validate"], vec!["index", "--check"], vec!["index"]] {
                args.push(scratch.path().to_str().unwrap());
                let (ok, output) = run_specful(&args);
                assert!(!ok, "{args:?} accepted {relative}");
